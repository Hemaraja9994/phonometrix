...
export default async function grokPwaMiddleware(
  ...
  if (
    ...
    const injector = createHeadInjector({ host: requestHost(event), site: grokOgIdentity.site });
    ...
    return new Response(transformed, { status: result.status, statusText: result.statusText, headers });
  }
  return result;
}
